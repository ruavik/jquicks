...
/*SRVDEF*/
    ...
    public function _insert_userfiles_model($args) {
        ...
        if(self::$this->db->query($sql)===false) self::_dbError();

    }
    public function _remove_userfiles_model($args) {
        list($dir, $fname) = $this->_getFileName($args['index'],"");
        //удаляем все файлы, которые начинаются на $fname, т.е. и тумбы и т.д.
        array_map("unlink", glob($this->project->path . '/data/' . $this->name . '/' . $dir. '/' . $fname."*"));
